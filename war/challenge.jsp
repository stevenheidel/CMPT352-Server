<!DOCTYPE html>

<%@ page contentType="text/html;charset=UTF-8" language="java"%>

<html>
<head>
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
<title>Challenge | QRMOR</title>

<link rel="stylesheet" type="text/css" href="static/style.css" />
</head>

<body>
	<div id="wrap">
		<div id="header">
			<div id="header-content">
				<h1>Challenge Instructions</h1>

				<p>Try to log in as Fred: (username: fred , password: fred)</p>
				<p>Tim Horton's gift card to anyone who can!</p>
				<p>If you notice any security problems or have any suggestions/questions, send an email to steven -at- heidel.ca</p>
				<p>The app is available on the Android Market, just search "QRMOR"</p>
				<p><a href="https://market.android.com/details?id=com.qrmor">Direct Link</a></p>
			</div>
		</div>

		<div class="line"></div>
	</div>
</body>
</html>